...
bool StoryLevelsScene::init(){
	...
	for (int i = 0; i < m_levels->size(); i++) {
		UIButton * btn = UIButton::node(m_levels->at(i)->chapter.c_str(), this, menu_selector(StoryLevelsScene::levelSelectedTap));
		menu->addChild(btn,i);
		btn->setPosition(CCPoint(200+(i%7* 150), 500 - (int(i/8) * 150)));
		btn->setTag(i);
